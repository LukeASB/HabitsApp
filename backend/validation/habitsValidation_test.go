...
func Test_ValidateHabitName(t *testing.T) {
	testCases := []struct {
		...
		{
			name:    "Invalid Habit Name - numbers",
			habit:   data.Habit{Name: "Habit 01", Days: 1, DaysTarget: 30},
			wantErr: true,
		},
	}
...
func Test_ValidateHabitDays(t *testing.T) {
	testCases := []struct {
		name  string
		habit data.Habit
		...
		{
			name:    "Valid Habit Days",
			habit:   data.Habit{Name: "Good Habit Name", Days: 1, DaysTarget: 30},
			wantErr: false,
		},
		{
			name:    "Invalid Habit Days - Exceeds Max Days",
			habit:   data.Habit{Name: "Good Habit Name", Days: 99999, DaysTarget: 30},
			wantErr: true,
		},
		{
			name:    "Invalid Habit Days - Negative Days",
	...
	for _, val := range testCases {
		t.Run(val.name, func(t *testing.T) {
			got := validateHabitDays(val.habit.Days)
